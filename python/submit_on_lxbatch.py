...
for a in range(len(category)):
    for i in range(len(name)):
        ...
        command = "python python/produceWWNtuples.py -i root://eoscms//eos/cms/store/caf/user/lbrianza/WWReducedTree_MC74x/ -n ReducedSelection_"+name[i]+".root -o WWTree_"+name[i]+".root -l "+category[a]+" -w "+xSecWeight[i]+" -no "+N[i]+" -mass "+mass[i];
        print command;
