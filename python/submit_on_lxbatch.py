...
xSecWeight = ["4.76735", "1.16691", "0.377865", "0.144482", "0.0616708", "0.0288651", "0.0141334", "0.00751431",
              ...
              "20488.7", "1292.", "385.9", "47.9", "19.9", "831.76", "10.11", "216.99", "80.95", "38.09", "38.09", "63.21", "10.32", "22.82",
              "43.53", "10.96", "3.38", "831.76", "831.76",
              "20488.8", "63.21", "10.32", "22.82", "831.76", "831.76", "831.76",
              "216.99", "80.95", "38.09", "38.09",
...
for a in range(len(category)):
    for i in range(len(name)):
        ...
        command = "python python/produceWWNtuples.py -n "+name[i]+" -o WWTree_"+name[i]+" -l "+category[a]+" -w "+xSecWeight[i]+" -no "+N[i]+" -mass "+mass[i]+" --ismc True";
        print command;
...
for a in range(len(category)):
    for i in range(len(nameData)):
        ...
        outScript = open(fn+".sh","w");
        command = "python python/produceWWNtuples.py -n "+nameData[i]+" -o WWTree_"+nameData[i]+" -l "+category[a]+" -w 1. -no 1. -mass 0 --ismc False";
        print command;
        outScript.write('#!/bin/bash');
        outScript.write("\n"+'cd '+CMSSWDir);
        outScript.write("\n"+'eval `scram runtime -sh`');
